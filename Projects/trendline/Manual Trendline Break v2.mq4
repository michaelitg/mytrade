...
string comment="MTB";                     // comment to display in the order
extern int magic=636363;                   // magic number required if you use different settings on a same pair, same timeframe
...
extern bool usewfcc=false;                  //wait for candle to close after getting trade signal
extern bool usemsgbox=false;
...
bool startlong=false,startshort=false;
datetime cctime; //candle close tests
datetime tstart,tend,tfriday,tmonday,lastbuyopentime,tnews,lastsellopentime,time,time2,time3,time4,time5,time6,time7,lastorderclosetime,longemailtime,shortemailtime,timea;
...
total,totalbuy,totalsell,totalstopbuy,totalstopsell,totallimitbuy,totallimitsell,confirmation=0,ret=0;
...
//      double ma1a=iMA(NULL,0,maperiod1,0,MODE_SMA,PRICE_CLOSE,shift+1);
//      double ma2a=iMA(NULL,0,maperiod2,0,MODE_SMA,PRICE_CLOSE,shift+1);
      
//      if(ma1>ma2 && !(ma1a>ma2a) && timea!=Time[0])signal=1;
//      if(ma1<ma2 && !(ma1a<ma2a) && timea!=Time[0])signal=2;
//////////// User Code
 int signal=0;
 if (crossedLine("start long")){
      startlong=true;
      if (usemsgbox)  {Comment(" Taking LONG at opening of next candle","\n","A confirmation box will be displayed before entering order");}
      if (!usemsgbox) {Comment(" Taking LONG at opening of next candle","\n","A confirmation box will **NOT** be displayed ");}
      cctime=Time[0];
   }
 if (crossedLine("start short")){
      startshort=true;
      if (usemsgbox)  {Comment(" Taking SHORT at opening of next candle","\n","A confirmation box will be displayed before entering order");}
      if (!usemsgbox) {Comment(" Taking SHORT at opening of next candle","\n","A confirmation box will **NOT** be displayed ");}
      cctime=Time[0];
   }
 
if (usewfcc && cctime!=Time[0])
   {
   if (startlong)   {signal=1;startlong=false;}
   if (startshort)  {signal=2;startshort=false;} 
   }
...
// if (cctime !=Time[0]) cctime=Time[0];
///// end user code
 if (usemsgbox)
 {
      if(signal!=0){
         ...
         confirmation=0;
         if (signal==1) ret=MessageBox(" Open LONG  ?", "Question", MB_YESNO|MB_ICONQUESTION);
         if (signal==2) ret=MessageBox(" Open SHORT ?", "Question", MB_YESNO|MB_ICONQUESTION);
         if(ret==IDYES) confirmation=1;
         timea=Time[0];
      }
 }
...
//+----------------------------------------------------------------------------+
   ...
   if(signal==1 && buyaddstatus && barstatus && buyside && continuebuy && tradetime && nosleep && confirmation){
      if(reversesignals)sell=true;else buy=true;
      if(changedirection){continuebuy=false;continuesell=true;}
   }
   if(signal==2 && selladdstatus && barstatus && sellside && continuesell && tradetime && nosleep && confirmation){
