...
int deinit()
{
	...
	{
		string VerticalLineName = ObjectName(j);
	...
	for (j=ObjectsTotal()-1; j >= 0; j--)
	{
		string HeadlineName = ObjectName(j);
...
//	Print("FFCal NEW MINUTE...Refreshing News from XML file...");
	...
	sData = FileReadString(xmlHandle, 65536);	
   	if ((unsigned)StringLen(sData) < FileSize(xmlHandle))
...
void DisplayVerticalNews(string dispTitle1, string dispCountry1, int shift)
{    
...
void DisplayOldNews(string dispTitle1, string dispCountry1, int shift, datetime TheTime)
{     
	int   	BarShift;
	double 	Pivot;
	double	Height = 0.0;
	...
	//Print("Title recieved : " + dispTitle1 + " Country Recieved: " + dispCountry1 + " Date and time Recieved: " + TheTime);
	...
	{  
		...
		if (shift == 0)
		{
			...
			if (ObjectFind("Headline" + TheTime) == -1)
			{
				...
				ObjectSetText("Headline" + TheTime, "News: " + dispCountry1 + " " + dispTitle1, 
							VertTxtSize, "Arial", VertTxtColor);
			}            
		}
		...
		else 
		{
			...
			if (ObjectFind("Headline" + TheTime + "s") == -1)
			{
				...
				ObjectSetText("Headline" + TheTime + "s", "News: " + dispCountry1 + " " + dispTitle1, 
								VertTxtSize, "Arial", VertTxtColor);
			}   
		} // end of "if shift)
